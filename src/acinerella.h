...
typedef int CALL_CONVT (*ac_read_callback)(void *sender, uint8_t *buf,
                                           int size);
/*Callback function used to ask the application to seek. return 0 if succeed ,
 * -1 on failure.*/
...
// Memory manager function. This allows you to bind the library to your
// applications memory interface.
/*extern void CALL_CONVT ac_mem_mgr(ac_malloc_callback, ac_realloc_callback,
 * ac_free_callback);*/
...
to seek a single audio/video stream
but seeks the whole file forward. The stream number paremter (nb) is only used
for the timecode reference.
The parameter "dir" specifies the seek direction: 0 for forward, -1 for
backward.
The target_pos paremeter is in milliseconds. Returns 1 if the functions
succeded.*/
extern int CALL_CONVT
